            ...
            if (isUndefined(backupError)) {
                downloadFiles(job, function (downloadError) {
                    if (isUndefined(downloadError)) {
                                            ...
                                            if (isUndefined(err)) {
                                                ...
                                                rollbackFiles(job, function (rollbackError) {
                                                    if (isUndefined(rollbackError)) {
                                                        ...
                                                        job.failed({
                                                            operation: job.operation, errorCode: 'ERR_UNEXPECTED_PACKAGE_EXIT',
                                                            errorMessage: errorToString(err), rollbackError: errorToString(rollbackError)
                                                        }, showJobsError);
                                                    }
                                                });
                                            }
                        ...
                        rollbackFiles(job, function (rollbackError) {
                            job.failed({
                                operation: job.operation, errorCode: 'ERR_DOWNLOAD_FAILED', downloadError: errorToString(downloadError),
                                statusCode: downloadError.statusCode, rollbackError: errorToString(rollbackError)
                            }, showJobsError);
                        });
                    }
                });
